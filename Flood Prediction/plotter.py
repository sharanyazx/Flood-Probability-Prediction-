...
for filename in filenames:
    ...
    data1 = pd.read_excel("Dataset/" + filename + ".xlsx")# Fill missing values with mean
    ...
    data1.drop(['Date', 'Flood'], axis=1, inplace=True)
    # Split the data
    ...
    # Summary plots
    shap.summary_plot(shap_values, x_test, show=True, feature_names=x_test.columns, class_names=['Low', 'High'])
    plt.savefig("figures/"+filename+"_dot_plot.png")
    shap.summary_plot(shap_values, x_test, plot_type='bar', show=True, feature_names=x_test.columns, class_names=['Low', 'High'])
    plt.savefig("figures/"+filename+"_bar_plot.png")
